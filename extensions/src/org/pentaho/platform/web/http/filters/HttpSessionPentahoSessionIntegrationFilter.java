 ...
 * <code>IPentahoSession</code> that should be stored against the
 * <code>PentahoSessiontHolder</code> for the duration of the web request. At
 * the end of the web request, any updates made to the
 * <code>PentahoSessionHolder</code> will be persisted back to the
 ...
 * the current Pentaho session in <code>PentahoSessionHolder</code> is not
 * null. This avoids needless
 * <code>HttpSession</code> creation, but automates the storage of changes
 * made to the <code>PentahoSessionHolder</code>. There is one exception to
 ...
 * irrespective of normal session-minimization logic (the default is
 ...
 * memory and ensure all classes using the <code>PentahoSessionHolder</code>
 * are designed to have no persistence of the Pentaho session
...
public class HttpSessionPentahoSessionIntegrationFilter implements Filter, InitializingBean {
   ...
   * Otherwise, {@link IPentahoSession#setAuthenticated(String)} is not called. This is necessary for code that 
   * calls {@link IPentahoSession#isAuthenticated()} in anonymous-only or mixed (i.e. anonymous and non-anonymous) 
   ...
   * TODO mlowery move this somewhere else
   */
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
    ...
    OnRedirectUpdateSessionResponseWrapper responseWrapper = new OnRedirectUpdateSessionResponseWrapper(httpResponse,
        httpRequest, httpSessionExistedAtStartOfRequest);
   ...
   *
   * @param pentahoSession the Pentaho session obtained from the PentahoSessionHolder after the request has
   *        been processed by the filter chain. PentahoSessionHolder.getSession() cannot be used to obtain
   *        the Pentaho session as it has already been cleared by the time this method is called.
   ...
   * the <code>IPentahoSession</code> when a <code>sendError()</code> or <code>sendRedirect</code>
   * happens. The class contains the fields needed to call
   * <code>storePentahoSessionInHttpSession()</code>
   */
